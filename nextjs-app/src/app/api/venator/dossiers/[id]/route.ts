...
import { detailDossier, cloreDossier } from '@/lib/venator/services/dossiers-service'
import { VenatorError, httpStatus } from '@/lib/venator/services/errors'

const cloreSchema = z.object({ action: z.literal('clore') })
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  const auth = await requireVenatorRole('admin')
  if (!auth.ok) return auth.response
  const parsed = cloreSchema.safeParse(await req.json().catch(() => null))
  if (!parsed.success) return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 })
  try {
    const dossier = await cloreDossier(createVenatorAdminClient(), params.id)
