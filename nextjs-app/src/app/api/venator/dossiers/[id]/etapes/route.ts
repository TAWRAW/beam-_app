...
import { ajouterEtape, majEtape, supprimerEtape } from '@/lib/venator/services/dossiers-service'
...
const etapeDeleteSchema = z.object({ etape_id: z.string().uuid() })
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!auth.ok) return auth.response
  const parsed = etapeDeleteSchema.safeParse(await req.json().catch(() => null))
  if (!parsed.success) return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 })
  try {
    await supprimerEtape(createVenatorAdminClient(), parsed.data.etape_id)
