...
import { creerTicket, listerTickets } from '@/lib/venator/services/tickets-service'
import { ticketCreateSchema } from '@/lib/venator/types'
...
export async function GET(req: NextRequest) {
  ...
  const tickets = await listerTickets(db, {
    copro_id: sp.get('copro_id') ?? undefined,
    dossier_id: sp.get('dossier_id') ?? undefined,
...
export async function POST(req: NextRequest) {
  ...
  try {
    const ticket = await creerTicket(createVenatorAdminClient(), parsed.data)
