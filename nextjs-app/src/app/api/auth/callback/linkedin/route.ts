...
// Endpoint: GET /api/auth/callback/linkedin
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (userError || !user) {
      console.error('No authenticated user for LinkedIn callback')
      return NextResponse.redirect(new URL('/auth/login', request.url))
    }

    // Vérifier le token CSRF
    const csrfState = await verifyCSRFToken(state, user.id, 'linkedin')

    if (!csrfState) {
      console.error('Invalid or expired CSRF token for LinkedIn OAuth')
    ...
    console.log(`✅ CSRF verified for user ${user.id} (LinkedIn)`)

    // Initialiser le handler OAuth LinkedIn
    const oauthHandler = getOAuthHandler('linkedin')
    ...
    if (dbError) {
      console.error('Failed to save LinkedIn integration:', dbError)
      throw new Error('Failed to save integration to database')
    }

    console.log(`✅ LinkedIn integration saved for user ${user.id}`)
